...
Entry point: `POST /api/invoices` inserts a `vendor_bills` row (status=`processing`) and returns immediately (see §5 for why this is async). The pipeline then runs 7 stages in order for a normal invoice (the schema's 8th `node_id` value, `audit_assemble`, belongs to workflow #6, not this pipeline — see AUDIT.md). Each stage writes exactly one `decisions` row, using the literal `node_id` named in bold below (these are the same strings as the schema's CHECK constraint, BUILD.md §2 — an earlier draft of this list used prose names like "Pre-Match Validation" and "Post/Route" that never matched the actual stored value; fixed now):

1. **`extract`** — for the ~28 structured/JSON demo fixtures, this is a pure parse into `vendor_bill_lines` (no LLM). For the 1-2 realistic PDF/email samples, a single LLM call extracts fields and reports its own confidence.
2. **`validate`** — Layer 1, deterministic, per spec §1.2: OCR/readability, exact-duplicate check, vendor-active status, currency-supported check, invoice-date sanity, mandatory-fields. If a **hard, unparseable-document** failure fires here or in `extract` (`EXC-LAYOUT` at its strictest — OCR confidence so low, or so many required fields missing, that no match is even attempted, per the spec's own "cannot proceed without parseable data"), or `EXC-DUPLICATE`/`EXC-FRAUD_BANK`/`EXC-NO_PO` fire (spec §4.3's cascade-stop list), the pipeline halts here and jumps straight to `policy`. **Clarifying a real ambiguity found on review**: a *soft* low-confidence-but-parseable extraction (DESIGN.md's `EXC-LAYOUT` framing that's "subject to the learning-loop override") is NOT this hard cascade-stop — it's a confidence flag that rides along into `match` and `investigate` normally, specifically so `investigate` can call `recall_vendor_corrections` and clear it if this vendor's layout is already a known-normal pattern. Only a genuinely unparseable document skips straight to `policy`; a "looks unusual but was readable" one does not.
...
4. **`investigate`** — Layer 2. Receives the `MatchResult`. For exceptions that need gathered context (duplicate-suspected beyond exact match, vendor-trust judgment, a non-standard-layout override via a learned correction), it calls tools (§3) in a genuine multi-turn loop, then writes a rationale + confidence + evidence citations via `submit_investigation` (ALGORITHMS.md §4). For exceptions the deterministic engine already resolved unambiguously (e.g., a clean quantity match), this stage is a fast pass-through with no tool calls at all — don't force LLM involvement where the spec already produced a certain answer.
5. **`verify`** — the TensorMux second-opinion pass, invoked only where a second judgment genuinely adds value: `Escalate L2` and `Block+Fraud-Flag` decisions (spec §3.2/§3.3), where the underlying call involves real ambiguity. **Not** invoked for a plain, rule-triggered `Block` with nothing to adjudicate (e.g. `EXC-LAYOUT`'s hard cascade-stop, or `EXC-DUPLICATE`'s "always block, no exceptions") — there's no judgment call to double-check there, just a rule that already fired deterministically. BUILD.md §12's Tier-2 UI grouping covers all three spec actions (`Escalate L2`/`Block`/`Block+Fraud-Flag`) for *display* purposes, but this stage's actual trigger condition is narrower and is what's authoritative. Disagreement between the two models forces escalation rather than trusting either.
...
7. **`audit`** — auto-approve writes the two-entry journal-entry pair (DESIGN.md §5.3) and updates status; escalate/block creates a `reviews` queue entry. This stage computes and writes the hash-chained `decisions.hash`, closing the loop for that invoice's current pass.

Cross-cutting, not a stage of its own: every one of the 7 row-inserts above publishes an SSE event (§4) so the swimlane UI updates live, in real time, as it actually happens — not a canned animation.
...
// get_policy
{ "type":"function", "name":"get_policy", "strict":true,
  "description":"Return the current tiered policy matrix (auto-approve/escalate/block thresholds per exception type) as structured data. Call this if you need to check a threshold explicitly rather than assume one — the policy engine (a separate, deterministic stage) is the actual authority, but you may reference it for context in your rationale.",
  "parameters":{"type":"object","properties":{},"required":[],"additionalProperties":false} }
```

**Tool count, corrected**: 7 evidence-gathering tools (all of the above) + 3 structured-output submission tools (ALGORITHMS.md §4: `submit_investigation`, `submit_verification`, `submit_extraction`) = **10 total**, not 9 — an earlier draft undercounted by silently dropping `get_policy`. `lib/agent/tools.ts` holds all 10.
...
- **Transactional writes**: the decision-row insert, any journal-entry postings, and the hash-chain update for a single stage are wrapped in one Postgres transaction (`sql.begin(...)`), so a crash mid-stage can't leave a half-posted invoice or a broken hash chain.
- **Database, updated**: this project runs on **Supabase Postgres**, not SQLite — switched deliberately once live web hosting became a requirement, since a locally-embedded SQLite file doesn't reliably persist on serverless hosts (ephemeral filesystem per request). Use the **pooled** connection string (port 6543, pgbouncer "Transaction" mode) in `DATABASE_URL` when deployed to a serverless host — a direct connection (port 5432) per serverless instance will exhaust Postgres's connection limit fast.
- **Hash-chain integrity under concurrency, now real, not hypothetical**: unlike SQLite (single-connection, serializes writes automatically), Postgres genuinely allows concurrent writers — two `writeDecision()` calls at the same moment could both read the same "last hash" and both insert claiming it as their `prev_hash`, forking the chain. `writeDecision()` (`lib/ledger/decisions.ts`) wraps the read-last-hash-then-insert sequence in a transaction-scoped Postgres advisory lock (`pg_advisory_xact_lock`) to serialize it globally — this was flagged as a future concern in an earlier draft of this doc ("scaling to Postgres would need..."); it's built now, since that future arrived as soon as live hosting did.
...
3. Writes a **new** `decisions` row at the same node, with `reconsideration_of_id` pointing to the original, and the two flat fields `triggered_by_actor` / `triggered_by_question` (matching the schema, BUILD.md §2 — not a nested object). **The original row is never edited** — append-only applies to reconsideration exactly as it applies to postings.
...
| `/api/pbc` | GET/POST | Workflow #6 — list / create PBC items (AUDIT.md §6, merged into this consolidated contract — an earlier draft left these orphaned in AUDIT.md only) |
| `/api/pbc/:id` | GET | PBC item detail incl. its decisions chain |
| `/api/pbc/:id/assemble` | POST | Run the evidence assembler for this item |
| `/api/pbc/:id/review` | POST | Mark a PBC item submitted/accepted/exception |
